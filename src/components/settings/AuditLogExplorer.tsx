...
type Props = { variant: "full" | "admin" };

export default function AuditLogExplorer({ variant }: Props) {
  const { hasPermission } = useAuth();
  const isAdminView = variant === "admin";

  const canViewFull = hasPermission("audit.view");
  const canViewAdminOnly = hasPermission("audit.view_admins");
  const canAccess = isAdminView ? canViewFull || canViewAdminOnly : canViewFull;
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      if (isAdminView) params.set("adminActorsOnly", "true");
  ...
  if (!canAccess) {
    return (
      <div>
        <PageBreadCrumb pageTitle={isAdminView ? "Admin activity" : "Activity log"} />
        <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
          {isAdminView ? (
            <>
              You need <code className="text-xs">audit.view_admins</code> (monitor admins only) or{" "}
              <code className="text-xs">audit.view</code> (full log) to open this page.
            </>
          ) : (
            <>
  ...
  const userDropdownList = isAdminView ? adminUserOptions : users;
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-sm">
        {isAdminView && canViewFull && (
          <Link
            href="/settings/activity"
            className="font-medium text-brand-600 hover:underline dark:text-brand-400"
          >
            Open full activity log (all users) →
          </Link>
        )}
        {!isAdminView && canViewFull && (
          <Link
            ...
            className="font-medium text-brand-600 hover:underline dark:text-brand-400"
          >
            Admin activity (Admin accounts only) →
          </Link>
        )}
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {canListUsers ? (
            <div>
              ...
              >
                <option value="">{isAdminView ? "All Admin users" : "All users"}</option>
                {userDropdownList.map((u) => (
