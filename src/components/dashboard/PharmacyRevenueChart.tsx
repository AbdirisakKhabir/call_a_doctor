...
export default function PharmacyRevenueChart() {
  ...
  useEffect(() => {
    authFetch("/api/dashboard")
      ...
      .then((data) => {
        if (data?.charts?.pharmacyRevenueByMonth) {
          const chart = data.charts.pharmacyRevenueByMonth;
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
        <h3 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">Pharmacy Revenue (This Year)</h3>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      ...
      <div className="-ml-5 min-w-[500px] pl-2">
        <ReactApexChart options={options} series={[{ name: "Revenue", data: series }]} type="bar" height={280} />
      </div>
    </div>
  );
}
