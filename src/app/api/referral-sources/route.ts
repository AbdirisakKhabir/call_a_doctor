...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("Referral sources list error:", e);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const row = await prisma.referralSource.create({
      data: {
        name: name.trim(),
        sortOrder: order,
      },
    ...
    console.error("Create referral source error:", e);
    return NextResponse.json({ error: "Something went wrong" }, { status: 500 });
  }
}
