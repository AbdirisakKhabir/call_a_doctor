...
export default function ServiceConsumeReportPage() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(`/api/reports/service-consume?${params.toString()}`);
  ...
  if (!canView) {
    return (
      <div>
        <PageBreadCrumb pageTitle="Service consume report" />
  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-3xl text-sm text-gray-600 dark:text-gray-400">
        For a selected period: how many services were provided on <strong>completed</strong> bookings, and pharmacy
        products consumed as visit <strong>service disposables</strong> (internal stock movements).
      </p>
      ...
      <div className="no-print mt-6 flex flex-wrap items-end gap-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
        ...
        <DateField id="svc-consume-from" label="From" value={from} onChange={setFrom} appendToBody />
        <DateField id="svc-consume-to" label="To" value={to} onChange={setTo} appendToBody />
        <button
          type="button"
          onClick={() => void run()}
      ...
      {loading && !data ? (
        ...
        <div className="mt-8 space-y-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                ...
                <p className="mt-1 text-2xl font-semibold tabular-nums text-brand-600 dark:text-brand-400">
                  {data.totalServiceQuantity}
                </p>
                <p className="mt-0.5 text-xs text-gray-500">Sum of line quantities</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
                  Service revenue
                ...
                <p className="mt-1 text-xl font-semibold tabular-nums text-gray-900 dark:text-white">
                  ${data.totals.serviceRevenue.toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
                  Disposables (cost / retail)
                </p>
                <p className="mt-1 text-sm font-semibold tabular-nums text-gray-900 dark:text-white">
                  ${data.totals.disposablesCost.toFixed(2)} / ${data.totals.disposablesSellingValue.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white">Services provided</h2>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">One row per catalog service; quantity is total units in the period.</p>
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white">Disposables consumed (pharmacy inventory)</h2>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Internal stock deductions with purpose <code className="rounded bg-gray-100 px-1 dark:bg-gray-800">service_disposable</code> in the period. Cost / retail use current product prices.
            </p>
            <div className="mt-2 overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
              <Table>
                <TableHeader>
