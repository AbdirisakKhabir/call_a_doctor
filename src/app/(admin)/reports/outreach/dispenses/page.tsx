...
export default function OutreachDispensesReportPage() {
  const { hasPermission } = useAuth();
  const { data, loading } = useOutreachReport();
  const rows = data?.dispensesToPatients ?? [];
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/3">
          <div className="border-b border-gray-100 bg-gray-50/90 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/40">
            <p className="text-sm font-medium text-gray-900 dark:text-white">
              Total emergency medication value: <span className="font-mono tabular-nums">${total.toFixed(2)}</span>
            </p>
          </div>
            ...
            <table className="w-full min-w-[800px] text-left text-sm">
              ...
              <tbody>
                {rows.map((d) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs text-gray-600 dark:text-gray-400">{d.id}</td>
                    <td className="px-4 py-3 text-gray-800 dark:text-gray-200">
                      {new Date(d.createdAt).toLocaleString()}
                    </td>
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{d.team.name}</td>
                    <td className="px-4 py-3 text-gray-800 dark:text-gray-200">
                      {d.patient.name}
                      <span className="ml-2 text-xs text-gray-500">{d.patient.patientCode}</span>
                    </td>
          ...
          {rows.length === 0 && (
            <p className="px-4 py-10 text-center text-sm text-gray-500">No emergency medication entries this period.</p>
          )}
        </div>
      )}
    </div>
