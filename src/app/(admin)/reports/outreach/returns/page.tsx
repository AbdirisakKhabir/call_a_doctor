...
export default function OutreachReturnsReportPage() {
  const { hasPermission } = useAuth();
  const { data, loading } = useOutreachReport();
  const rows = data?.returnsToPharmacy ?? [];
  const total = rows.reduce((s, r) => s + r.totalAmount, 0);

  if (!hasPermission("pharmacy.view")) {
    return (
      <div>
        <PageBreadCrumb pageTitle="Returns to pharmacy" />
  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-2xl text-sm text-gray-600 dark:text-gray-400">
        Medication returned from outreach bags to shelf stock. Team accounts receivable is reduced by the value
        of each return.
      </p>
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/3">
          <div className="border-b border-gray-100 bg-gray-50/90 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/40">
            <p className="text-sm font-medium text-gray-900 dark:text-white">
              Total return value: <span className="font-mono tabular-nums">${total.toFixed(2)}</span>
            </p>
          </div>
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900/50">
                  <th className="px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">Return #</th>
              ...
              <tbody>
                {rows.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{r.team.name}</td>
          ...
          {rows.length === 0 && (
            <p className="px-4 py-10 text-center text-sm text-gray-500">No returns recorded this period.</p>
          )}
        </div>
      )}
    </div>
